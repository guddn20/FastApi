...
#Path : 함수가 실행될 때 타입을 검사, 정리해주는 역할
#Field : 백엔드 서버에 들어올 데이터를 명시

# 2. FastAPI 애플리케이션 객체 생성
# http://127.0.0.1:8000은 이제 'app'이라는 이름으로 부름
app = FastAPI()
...
@app.get('/course/{course_id}') # TODO: 변경된 경로 규칙을 문자열로 작성하세요  # ... , (필수 파라미터)
